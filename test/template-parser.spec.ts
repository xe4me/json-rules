...
describe("TemplateParser", () => {
  ...
  describe("hasTemplateVariables", () => {
    it("should detect template variables in strings", () => {
      ...
      expect(parser.hasTemplateVariables("Value: {value1} and {value2}")).toBe(
        true
      );
    });
    ...
    it("should not detect template variables in non-template strings", () => {
      ...
      expect(
        parser.hasTemplateVariables("curly braces but no template { }")
      ).toBe(true); // { } is detected as template but will be rejected by validator
  ...
  describe("extractTemplateVariables", () => {
    ...
    it("should extract multiple template variables", () => {
      const result = parser.extractTemplateVariables(
  ...
  describe("resolveTemplateValue", () => {
    ...
    it("should resolve nested field references", () => {
      expect(
        parser.resolveTemplateValue("{profile.firstName}", mockCriteria)
      ).toBe("John");
      expect(
        parser.resolveTemplateValue("{profile.lastName}", mockCriteria)
      ).toBe("Doe");
      expect(
        parser.resolveTemplateValue("{profile.settings.theme}", mockCriteria)
      ).toBe("dark");
    });

    it("should resolve multiple template variables in a string", () => {
      const result = parser.resolveTemplateValue(
        "Hello {name}, you are {age} years old!",
        mockCriteria
      );
    ...
    it("should resolve mixed template variables", () => {
      const result = parser.resolveTemplateValue(
        "{profile.firstName} {profile.lastName} - Age: {age}",
        mockCriteria
      );
    ...
    it("should return non-template values unchanged", () => {
      expect(parser.resolveTemplateValue("regular string", mockCriteria)).toBe(
        "regular string"
      );
  ...
  describe("validateTemplateVariables", () => {
    ...
    it("should validate multiple existing template variables", () => {
      const result = parser.validateTemplateVariables(
        "Hello {name}, you are {age} years old!",
        mockCriteria
      );
      expect(result.isValid).toBe(true);
    ...
    it("should detect mixed valid and invalid template variables", () => {
      const result = parser.validateTemplateVariables(
        "Hello {name}, your {missing} is {age}",
        mockCriteria
      );
      expect(result.isValid).toBe(false);
