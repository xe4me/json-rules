...
// Import phone validators to register them
...
describe("Advanced Validators", () => {
  describe("Email Validation", () => {
    it("should validate basic email addresses", async () => {
      ...
      expect(await JsonRules.evaluate(rule, { email: "user.name+tag@domain.co.uk" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { email: "invalid-email" })).toBe(false);
      expect(await JsonRules.evaluate(rule, { email: "@domain.com" })).toBe(false);
    ...
    it("should validate emails with configuration", async () => {
      ...
      expect(await JsonRules.evaluate(rule, { email: "test@example.com" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { email: "test@tempmail.com" })).toBe(false);
      expect(await JsonRules.evaluate(rule, { email: "Name <test@example.com>" })).toBe(false);
    });
    ...
    it("should validate emails with whitelist", async () => {
      ...
      expect(await JsonRules.evaluate(rule, { email: "user@company.com" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { email: "user@partner.org" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { email: "user@external.com" })).toBe(false);
    });
  ...
  describe("Phone Validation", () => {
    it("should validate US phone numbers", async () => {
      ...
      expect(await JsonRules.evaluate(rule, { phone: "+1 202 456 1111" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { phone: "(202) 456-1111" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { phone: "202-456-1111" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { phone: "2024561111" })).toBe(true);
    ...
    it("should validate GB phone numbers", async () => {
      ...
      expect(await JsonRules.evaluate(rule, { phone: "+44 7700 900123" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { phone: "07700 900123" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { phone: "+1 202 456 1111" })).toBe(false);
    });
    ...
    it("should validate German phone numbers", async () => {
      ...
      expect(await JsonRules.evaluate(rule, { phone: "+49 151 12345678" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { phone: "0151 12345678" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { phone: "+1 202 456 1111" })).toBe(false);
    });
    ...
    it("should throw error for unregistered locale", async () => {
      ...
      await expect(JsonRules.evaluate(rule, { phone: "+33 123456789" }))
  ...
  describe("URL Validation", () => {
    it("should validate basic URLs", async () => {
      const rule: Rule = {
        conditions: { all: [{ field: "url", operator: "is URL", value: { requireTld: false } }] },
      };

      expect(await JsonRules.evaluate(rule, { url: "https://www.example.com" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { url: "http://localhost:3000" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { url: "ftp://files.example.com" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { url: "invalid-url" })).toBe(false);
    ...
    it("should validate URLs with protocol restrictions", async () => {
      ...
      expect(await JsonRules.evaluate(rule, { url: "http://example.com" })).toBe(false);
      expect(await JsonRules.evaluate(rule, { url: "ftp://example.com" })).toBe(false);
    });
  ...
  describe("UUID Validation", () => {
    it("should validate UUIDs", async () => {
      ...
      expect(await JsonRules.evaluate(rule, { id: "6ba7b810-9dad-11d1-80b4-00c04fd430c8" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { id: "invalid-uuid" })).toBe(false);
      expect(await JsonRules.evaluate(rule, { id: "550e8400-e29b-41d4-a716" })).toBe(false);
    });
  ...
  describe("EAN Validation", () => {
    it("should validate EAN codes", async () => {
      const rule: Rule = {
        conditions: { all: [{ field: "barcode", operator: "is EAN", value: null }] },
      };

      expect(await JsonRules.evaluate(rule, { barcode: "4006381333931" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { barcode: "9780471117094" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { barcode: "1234567890123" })).toBe(false);
      expect(await JsonRules.evaluate(rule, { barcode: "invalid-ean" })).toBe(false);
    });
    ...
    it("should handle non-string values", async () => {
      ...
      expect(await JsonRules.evaluate(rule, { barcode: 4006381333931 })).toBe(false);
  ...
  describe("IMEI Validation", () => {
    it("should validate IMEI numbers", async () => {
      const rule: Rule = {
        conditions: { all: [{ field: "imei", operator: "is IMEI", value: {} }] },
      };

      expect(await JsonRules.evaluate(rule, { imei: "352099001761481" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { imei: "356938035643809" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { imei: "123456789012345" })).toBe(false);
      expect(await JsonRules.evaluate(rule, { imei: "invalid-imei" })).toBe(false);
    });
    ...
    it("should validate IMEI with hyphens", async () => {
      ...
      expect(await JsonRules.evaluate(rule, { imei: "35-209900-176148-1" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { imei: "352099001761481" })).toBe(true);
    });
  });
  ...
  describe("Unit Validation", () => {
    it("should validate length units", async () => {
      ...
      expect(await JsonRules.evaluate(rule, { distance: "10 meters" })).toBe(true);
      ...
      expect(await JsonRules.evaluate(rule, { distance: "5 seconds" })).toBe(false);
      expect(await JsonRules.evaluate(rule, { distance: "invalid" })).toBe(false);
    });

    it("should validate mass units", async () => {
      const rule: Rule = {
        conditions: { all: [{ field: "weight", operator: "is unit", value: "mass" }] },
      };
      ...
      expect(await JsonRules.evaluate(rule, { weight: "5 meters" })).toBe(false);
    });

    it("should validate temperature units", async () => {
      const rule: Rule = {
        conditions: { all: [{ field: "temp", operator: "is unit", value: "temperature" }] },
      };
      ...
      expect(await JsonRules.evaluate(rule, { temp: "100 meters" })).toBe(false);
    });

    it("should validate time units", async () => {
      const rule: Rule = {
        conditions: { all: [{ field: "duration", operator: "is unit", value: "time" }] },
      };

      expect(await JsonRules.evaluate(rule, { duration: "30s" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { duration: "5 minutes" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { duration: "2.5 hours" })).toBe(true);
    ...
    it("should handle scientific notation and decimals", async () => {
      ...
      expect(await JsonRules.evaluate(rule, { distance: "0.001 mm" })).toBe(true);
    });
  ...
  describe("Country Validation", () => {
    ...
    it("should validate country names", async () => {
      ...
      expect(await JsonRules.evaluate(rule, { country: "United States" })).toBe(true);
      ...
      expect(await JsonRules.evaluate(rule, { country: "US" })).toBe(false);
      expect(await JsonRules.evaluate(rule, { country: "Invalid Country" })).toBe(false);
    });
  ...
  describe("Domain Validation", () => {
    it("should validate domain names", async () => {
      const rule: Rule = {
        conditions: { all: [{ field: "domain", operator: "is domain", value: {} }] },
      };

      expect(await JsonRules.evaluate(rule, { domain: "example.com" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { domain: "subdomain.example.org" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { domain: "api.service.company.co.uk" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { domain: "invalid_domain" })).toBe(false);
      expect(await JsonRules.evaluate(rule, { domain: ".invalid" })).toBe(false);
    });
    ...
    it("should validate domains with configuration", async () => {
      ...
      expect(await JsonRules.evaluate(rule, { domain: "example.com" })).toBe(true);
      expect(await JsonRules.evaluate(rule, { domain: "localhost" })).toBe(false);
      expect(await JsonRules.evaluate(rule, { domain: "under_score.com" })).toBe(false);
    });
  });
